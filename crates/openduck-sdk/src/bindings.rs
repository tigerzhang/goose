...
impl From<openduck_providers::errors::ProviderError> for GooseError {
    fn from(error: openduck_providers::errors::ProviderError) -> Self {
        match error {
            openduck_providers::errors::ProviderError::Authentication(message) => {
                Self::Authentication { details: message }
            }
            openduck_providers::errors::ProviderError::ContextLengthExceeded(message) => {
                Self::ContextLengthExceeded { details: message }
            }
            openduck_providers::errors::ProviderError::RateLimitExceeded {
                retry_delay, ..
            } => {
            ...
            openduck_providers::errors::ProviderError::ServerError(message)
            | openduck_providers::errors::ProviderError::EndpointNotFound(message)
            | openduck_providers::errors::ProviderError::CreditsExhausted {
                details: message,
                ..
            } => Self::ProviderUnavailable { details: message },
            openduck_providers::errors::ProviderError::NetworkError(message)
                if is_timeout(&message) =>
            {
                Self::Timeout { details: message }
            }
            openduck_providers::errors::ProviderError::RequestFailed(message)
                if is_timeout(&message) =>
            {
                Self::Timeout { details: message }
            }
            openduck_providers::errors::ProviderError::ExecutionError(message)
...
impl openduck_providers::request_log::RequestLogger for RequestLoggerAdapter {
    fn start(
        &self,
    ) -> Result<
        Box<dyn openduck_providers::request_log::RequestLogHandle>,
...
impl openduck_providers::request_log::RequestLogHandle for RequestLogHandleAdapter {
...
pub fn install_request_logger(logger: Box<dyn RequestLogger>) -> Result<(), GooseError> {
    openduck_providers::request_log::install_logger(RequestLoggerAdapter {
...
impl Provider {
    ...
    pub async fn compact(
        ...
        let model = openduck_context_management::ProviderModel::new(
            self.handle.provider.clone(),
            ModelConfig::new(&model_name),
        );

        let summary = run_on_runtime(async move {
            openduck_context_management::summarize(&model, None, &templates, &messages).await
        })
        .await?
        .map_err(GooseError::generic)?;
...
impl From<CompactionTemplates> for openduck_context_management::Templates {
...
pub fn default_compaction_templates() -> CompactionTemplates {
    let templates = openduck_context_management::Templates::default();
    CompactionTemplates {
...
pub fn declarative_provider_from_json(json: String) -> Result<Arc<Provider>, GooseError> {
    let provider = openduck_providers::declarative::from_json(&json, None, EnvKeyResolver {})?;
    Ok(Provider::new(provider))
}

#[uniffi::export]
pub fn openai_default_model() -> String {
    openduck_providers::openai::OPEN_AI_DEFAULT_MODEL.to_string()
}
...
pub fn anthropic_default_model() -> String {
    openduck_providers::anthropic::ANTHROPIC_DEFAULT_MODEL.to_string()
}
...
pub fn anthropic_provider(
    ...
    .with_header(
        "anthropic-version",
        openduck_providers::anthropic::ANTHROPIC_API_VERSION,
    )?;
...
pub fn groq_provider(api_key: String) -> Result<Arc<Provider>, GooseError> {
    let json = openduck_providers::groq::JSON.replace("${GROQ_API_KEY}", &api_key);
    let provider = openduck_providers::declarative::from_json(
        &json,
        None,
        StaticKeyResolver {
...
fn groq_config() -> DeclarativeProviderConfig {
    openduck_providers::declarative::deserialize_provider_config(openduck_providers::groq::JSON)
...
impl openduck_providers::declarative::KeyResolver for StaticKeyResolver {
...
pub fn databricks_default_model() -> String {
    openduck_providers::databricks::DATABRICKS_DEFAULT_MODEL.to_string()
}
...
pub fn databricks_v2_default_model() -> String {
    openduck_providers::databricks_v2::DATABRICKS_V2_DEFAULT_MODEL.to_string()
}
