...
use anyhow::Result;
use openduck::agents::{Agent, AgentConfig, GoosePlatform};
use openduck::config::permission::PermissionManager;
use openduck::config::GooseMode;
use openduck::providers::{create, testprovider::TestProvider};
    ...
    F: Fn(&ScenarioResult) -> Result<()>,
{
    ...
    openduck::agents::moim::SKIP.with(|f| f.set(true));
    ...
    let scenario_model_config =
        openduck::model_config::model_config_from_user_config(&factory_name, config.model_name)?;
    agent
        .update_provider(
            provider_arc as Arc<dyn openduck::providers::base::Provider>,
