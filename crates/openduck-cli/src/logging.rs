use anyhow::Result;
use openduck::providers::utils::init_goose_request_log;
...
pub fn setup_logging(name: Option<&str>) -> &'static Result<()> {
    INIT.get_or_init(|| {
        ...
        let config = openduck::logging::LoggingConfig {
            component: "cli",
            name,
            extra_directives: &["openduck_cli=info"],
            console: false,
            json: true,
        };
        let subscriber = openduck::logging::build_logging_subscriber(&config)?;
...
#[cfg(test)]
mod tests {
    use openduck::tracing::langfuse_layer;
    use std::env;
    ...
    fn test_log_directory_creation() {
        ...
        let log_dir = openduck::logging::prepare_log_directory("cli", true).unwrap();
    ...
    async fn test_default_filter_avoids_debug_by_default() {
        ...
        assert!(openduck::logging::build_logging_subscriber(&config).is_ok());
    }
}
