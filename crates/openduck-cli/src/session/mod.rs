...
use console::Color;
use openduck::agents::platform_extensions::developer::shell::{
    parse_shell_output_notification, ShellOutputNotificationParams, ShellOutputStream,
};
use openduck::agents::AgentEvent;
use openduck::agents::SUBAGENT_TOOL_REQUEST_TYPE;
use openduck::permission::permission_confirmation::PrincipalType;
use openduck::permission::Permission;
use openduck::permission::PermissionConfirmation;
use openduck::providers::base::Provider;
use openduck::providers::base::ProviderUsage;
use openduck::utils::safe_truncate;

use anyhow::{Context, Result};
use completion::GooseCompleter;
use input::InputResult;
use openduck::agents::extension::{Envs, ExtensionConfig, PLATFORM_EXTENSIONS};
use openduck::agents::types::RetryConfig;
use openduck::agents::{
    context_management_unsupported_message, Agent, SessionConfig, COMPACT_TRIGGERS,
};
use openduck::config::extensions::name_to_key;
...
use openduck::config::providers;
use openduck::conversation::message::{ActionRequiredData, Message, MessageContent};
use openduck::providers::inventory::ProviderInventoryService;
...
pub async fn classify_planner_response(
    ...
    model_config: openduck_providers::model::ModelConfig,
    ...
    let (result, _usage) = openduck::session_context::with_session_id(
...
impl CliSession {
    ...
    pub fn parse_stdio_extension(extension_command: &str) -> Result<ExtensionConfig> {
        ...
        let mut parts = openduck::utils::split_command_args(command)?;
        ...
        Ok(ExtensionConfig::Stdio {
            ...
            envs: Envs::new(envs),
            env_keys: Vec::new(),
            description: openduck::config::DEFAULT_EXTENSION_DESCRIPTION.to_string(),
            timeout: Some(openduck::config::DEFAULT_EXTENSION_TIMEOUT),
            cwd: None,
            bundled: None,
    ...
    pub async fn add_streamable_http_extension(&mut self, extension_url: String) -> Result<()> {
        let config = Self::parse_streamable_http_extension(
            &extension_url,
            openduck::config::DEFAULT_EXTENSION_TIMEOUT,
        );
        self.add_and_persist_extensions(vec![config]).await
    }
    ...
    pub async fn interactive(&mut self, prompt: Option<String>) -> Result<()> {
        let banners = self
            .agent
            .emit_hook_with_banners(openduck::hooks::HookEvent::SessionStart, &self.session_id)
            .await;
        if !banners.is_empty() {
        ...
        self.agent
            .emit_hook(openduck::hooks::HookEvent::SessionEnd, &self.session_id)
            .await;
                    ...
                    None => {
                        output::render_error(
                            "No editor found. Set one with:\n  \
                                 openduck configure set goose_prompt_editor \"vim\"\n  \
    ...
    async fn handle_model(&mut self, options: input::ModelCommandOptions) -> Result<()> {
        ...
        let target_entry = match openduck::providers::get_from_registry(target_provider_name).await
        {
            Ok(entry) => entry,
            Err(_) => {
                output::render_error(&format!(
        ...
        let new_provider = match openduck::providers::create(target_provider_name, extensions).await
        {
            Ok(p) => p,
            Err(e) => {
                output::render_error(&format!(
                    "Cannot switch to provider '{}': {}\n\
                         Set credentials via `openduck configure` or the appropriate environment variable.\n\
    ...
    async fn handle_clear(&mut self) -> Result<()> {
        ...
        if let Err(e) = self
            ...
            .usage(openduck_providers::conversation::token_usage::Usage::new(
                Some(0),
                Some(0),
                Some(0),
            ))
    ...
    async fn handle_list_skills(&mut self) -> Result<()> {
        ...
        use openduck::custom_requests::SourceType;
        use openduck::skills::list_installed_skills;
    ...
    async fn plan_with_reasoner_model(
        ...
        output::show_thinking();
        let (plan_response, _usage) = openduck::session_context::with_session_id(
    ...
    pub async fn update_completion_cache(&mut self) -> Result<()> {
        let prompts = self.agent.list_extension_prompts(&self.session_id).await;
        let all_providers = openduck::providers::providers().await;
    ...
    pub async fn get_session(&self) -> Result<openduck::session::Session> {
        self.agent
            .config
            .session_manager
...
async fn create_successor_session(
    ...
    old_session: &openduck::session::Session,
...
fn handle_agent_error(e: &anyhow::Error, is_stream_json_mode: bool) {
    ...
    if e.downcast_ref::<openduck_providers::errors::ProviderError>()
        .map(|provider_error| {
            matches!(
                provider_error,
                openduck_providers::errors::ProviderError::ContextLengthExceeded(_)
            )
        })
        .unwrap_or(false)
    {
...
async fn get_reasoner(
) -> Result<(Arc<dyn Provider>, openduck_providers::model::ModelConfig), anyhow::Error> {
    use openduck::providers::create;
    ...
    let provider = if let Ok(provider) = config.get_param::<String>("GOOSE_PLANNER_PROVIDER") {
        ...
        config
            .get_goose_provider()
            .expect("No provider configured. Run 'openduck configure' first")
    };
    ...
    let model = if let Ok(model) = config.get_param::<String>("GOOSE_PLANNER_MODEL") {
        ...
        config
            .get_goose_model()
            .expect("No model configured. Run 'openduck configure' first")
    };
    ...
    let model_config =
        openduck::model_config::model_config_from_user_config(&provider, model.as_str())?
            .with_context_limit(planner_context_limit);
    let extensions = openduck::config::extensions::get_enabled_extensions_with_config(config);
...
fn build_switched_model_config(
    provider_name: &str,
    model_name: &str,
    current_model_config: &openduck_providers::model::ModelConfig,
) -> Result<openduck_providers::model::ModelConfig> {
    openduck::model_config::model_config_from_user_config(provider_name, model_name)
        .map(|config| {
            config
...
mod tests {
    use super::*;
    use openduck::agents::extension::Envs;
    use openduck::config::ExtensionConfig;
    ...
    fn planner_history_excludes_turn_context_events() {
        use openduck::conversation::message::MessageMetadata;
    ...
    fn test_build_switched_model_config_rebuilds_target_model_settings() {
        ...
        let current_model_config = openduck_providers::model::ModelConfig {
        ...
        let expected = openduck_providers::model::ModelConfig::new("gpt-5.4")
    ...
    fn test_build_switched_model_config_detects_effort_suffix_change() {
        ...
        let current = openduck_providers::model::ModelConfig::new("gpt-5.4-high")
            .with_canonical_limits("openai");
        assert_eq!(current.model_name, "gpt-5.4");
        assert_eq!(
            current.thinking_effort(),
            Some(openduck_providers::thinking::ThinkingEffort::High)
        );
    ...
    fn test_split_command_args_windows_paths() {
        assert_eq!(
            openduck::utils::split_command_args(r"C:\tools\mcp.exe --arg value").unwrap(),
            vec![r"C:\tools\mcp.exe", "--arg", "value"]
        );
        assert_eq!(
            openduck::utils::split_command_args(r#""C:\Program Files\server\mcp.exe" --arg"#)
                .unwrap(),
    ...
    fn test_split_command_args_unmatched_quote() {
        assert!(openduck::utils::split_command_args(r#""unmatched"#).is_err());
    }
    ...
    async fn new_session_inherits_provider_model_and_working_dir() {
        ...
        let old = sm
            .create_session(
                ...
                "CLI Session".to_string(),
                openduck::session::SessionType::User,
                GooseMode::Auto,
            )
            .await
            .unwrap();
        ...
        sm.update(&old.id)
            ...
            .model_config(openduck_providers::model::ModelConfig::new("test-model"))
            .accumulated_usage(openduck_providers::conversation::token_usage::Usage::new(
                Some(100),
                Some(50),
                Some(150),
            ))
        ...
        let mut extension_data = openduck::session::ExtensionData::new();
