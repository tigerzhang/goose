...
fn additional_builtin_help() -> String {
    ...
    openduck::agents::execute_commands::list_commands()
        .iter()
...
fn print_editor_help() {
  ...
  Configure editor: openduck configure set goose_prompt_editor \"vim\"
  ...
  To use inline prompts with on-demand /edit: openduck configure set goose_prompt_editor_always false"
    );
}
...
mod tests {
    ...
    fn help_lists_builtin_agent_commands() {
        ...
        for command in openduck::agents::execute_commands::list_commands() {
            assert!(
