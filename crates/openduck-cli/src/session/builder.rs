...
use console::style;
use openduck::agents::{Agent, Container, ExtensionError};
use openduck::config::extensions::name_to_key;
use openduck::config::resolve_extensions_for_new_session;
use openduck::config::{Config, ExtensionConfig, GooseMode};
use openduck::model_config::model_config_from_user_config;
use openduck::providers::create;
use openduck::recipe::Recipe;
use openduck::session::session_manager::SessionType;
use openduck::session::EnabledExtensionsState;
...
async fn resolve_provider_and_model(
    ...
    saved_model_config: Option<openduck_providers::model::ModelConfig>,
    ...
    let provider_name = session_config
        ...
        .unwrap_or_else(|| {
            output::render_error("No provider configured. Run 'openduck configure' first.");
            process::exit(1);
        });
    ...
    let configured_provider_model = session_config.provider.as_ref().and_then(|_| {
        openduck::config::get_provider_entry(config, &provider_name)
    ...
    {
        Some(
            openduck::providers::get_from_registry(&provider_name)
                .await
                .unwrap_or_else(|e| {
    ...
    let model_name = session_config
        ...
        .unwrap_or_else(|| {
            output::render_error("No model configured. Run 'openduck configure' first.");
            process::exit(1);
        });
...
async fn resolve_session_id(
    session_config: &SessionBuilderConfig,
    session_manager: &openduck::session::session_manager::SessionManager,
...
async fn collect_extension_configs(
    ...
    if !session_config.no_profile && !session_config.resume && recipe_extensions.is_none() {
        ...
        all.extend(
            openduck::plugins::mcp_servers::enabled_plugin_mcp_servers(project_root.as_deref())
                .into_iter()
                .map(|config| (config.name(), config)),
        );
    }
...
pub async fn build_session(session_config: SessionBuilderConfig) -> CliSession {
    ...
    openduck::posthog::set_session_context("cli", session_config.resume);
                    ...
                    && is_provider_unavailable_error(&e) =>
            {
                ...
                match create(&fallback_provider, extensions_for_provider.clone()).await {
                    ...
                    Err(e2) => {
                        output::render_error(&format!(
                        "Error {}.\n\
                        Please check your system keychain and run 'openduck configure' again.\n\
...
mod tests {
    use super::*;
    use openduck::config::{set_provider_entry, ProviderEntry};
    ...
    fn saved_model_config(model_name: &str) -> openduck_providers::model::ModelConfig {
        openduck_providers::model::ModelConfig::new(model_name).with_merged_request_params(
    ...
    fn test_session_builder_config_creation() {
        let config = SessionBuilderConfig {
            ...
            streamable_http_extensions: vec![StreamableHttpOptions {
                ...
                timeout: openduck::config::DEFAULT_EXTENSION_TIMEOUT,
            }],
    ...
    async fn resume_provider_override_uses_target_provider_model() {
        ...
        let resolved = resolve_provider_and_model(
            ...
            Some(openduck_providers::model::ModelConfig::new(
                "claude-sonnet-4-6",
            )),
        )
        .await;
    ...
    async fn matching_environment_model_overrides_saved_model() {
        ...
        let resolved = resolve_provider_and_model(
            ...
            Some(openduck_providers::model::ModelConfig::new("saved-model")),
        )
        .await;
    ...
    async fn resume_provider_override_uses_target_provider_default_instead_of_active_model() {
        ...
        config
            ...
            .unwrap();
        let expected_model = openduck::providers::get_from_registry("openai")
            .await
            .unwrap()
            .metadata()
            .default_model
