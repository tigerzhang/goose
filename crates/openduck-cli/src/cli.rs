...
use openduck::agents::GoosePlatform;
use openduck::builtin_extension::register_builtin_extensions;
use openduck::config::{Config, GooseMode};
#[cfg(feature = "telemetry")]
use openduck::posthog::get_telemetry_choice;
use openduck::recipe::Recipe;
use openduck::source_roots::SourceRoot;
use openduck_mcp::mcp_server_runner::{serve, McpCommand};
use openduck_mcp::{AutoVisualiserRouter, ComputerControllerServer, MemoryServer, TutorialServer};
...
use openduck::agents::Container;
use openduck::session::session_manager::SessionType;
use openduck::session::SessionManager;
use std::io::Read;
...
#[derive(Parser)]
#[command(name = "openduck", author, version, display_name = "", about, long_about = None)]
pub struct Cli {
    #[command(subcommand)]
...
fn parse_streamable_http_extension(input: &str) -> Result<StreamableHttpOptions, String> {
    ...
    let (mut url, mut timeout) = (String::new(), openduck::config::DEFAULT_EXTENSION_TIMEOUT);
...
#[derive(Subcommand)]
enum SkillsCommand {
    /// List all skills available to the OpenDuck agent
    #[command(about = "List all skills available to the OpenDuck agent")]
    List,
}
...
#[derive(Subcommand)]
enum Command {
    /// Configure OpenDuck settings
    #[command(about = "Configure OpenDuck settings")]
    Configure {},

    /// Display OpenDuck configuration information
    #[command(about = "Display OpenDuck information")]
    Info {
    ...
    #[command(about = "Check that your OpenDuck setup is working")]
    Doctor {},

    /// Manage system prompts and behaviors
    #[command(about = "Run one of the mcp servers bundled with OpenDuck")]
    Mcp {
        #[arg(value_parser = clap::value_parser!(McpCommand))]
        server: McpCommand,
    },

    /// Run OpenDuck as an ACP (Agent Client Protocol) agent
    #[command(about = "Run OpenDuck as an ACP agent server on stdio")]
    Acp {
    ...
    /// Update the OpenDuck CLI version
    #[cfg(feature = "update")]
    #[command(about = "Update the OpenDuck CLI version")]
    Update {
        ...
        #[arg(
            ...
            long_help = "Update to the latest canary version of the OpenDuck CLI, otherwise updates to the latest stable version."
        )]
        canary: bool,

        /// Enforce to re-configure OpenDuck during update
        #[arg(short, long, help = "Enforce to re-configure OpenDuck during update")]
        reconfigure: bool,
    },

    /// Terminal-integrated session (one session per terminal)
    #[command(
        about = "Terminal-integrated OpenDuck session",
        long_about = "Runs an OpenDuck session tied to your terminal window.\n\
                      Each terminal maintains its own persistent session that resumes automatically.\n\n\
                      Setup:\n  \
                        eval \"$(openduck term init zsh)\"  # zsh/bash\n  \
                        let init = ($nu.cache-dir | path join \"openduck-term-init.nu\"); ^openduck term init nu | save --force $init; source $init\n\n\
                      Usage:\n  \
                        openduck term run \"list files in this directory\"\n  \
    ...
    /// Launch the OpenDuck terminal UI (TUI)
    #[cfg(feature = "tui")]
    #[command(
        about = "Launch the OpenDuck terminal UI",
        long_about = "Launch the OpenDuck terminal UI (the @aaif/goose npm package).\n\
                      \n\
                      Resolution order:\n  \
    ...
    /// runs the review through OpenDuck.
    #[command(about = "Review the current diff using OpenDuck")]
    Review {
...
enum TermCommand {
    ...
    #[command(
        ...
        long_about = "Prints shell configuration to set up terminal-integrated sessions.\n\
                      Each terminal gets a persistent OpenDuck session that automatically resumes.\n\n\
                      Setup:\n  \
                        echo 'eval \"$(openduck term init zsh)\"' >> ~/.zshrc\n  \
                        source ~/.zshrc\n\n\
                        Nushell:\n  \
                        let init = ($nu.cache-dir | path join \"openduck-term-init.nu\")\n  \
                        ^openduck term init nu | save --force $init\n  \
                        source $init\n\n\
                      With --default (anything typed that isn't a command goes to OpenDuck):\n  \
                        echo 'eval \"$(openduck term init zsh --default)\"' >> ~/.zshrc\n  \
                        ^openduck term init nu --default | save --force $init"
    )]
    Init {
        ...
        /// Make OpenDuck the default handler for unknown commands
        #[arg(
            long = "default",
            help = "Make OpenDuck the default handler for unknown commands",
            long_help = "When enabled, anything you type that isn't a valid command will be sent to OpenDuck. Supported for zsh, bash, and nu."
        )]
        default: bool,
    },
...
async fn handle_serve_command(args: ServeCommandArgs) -> Result<()> {
    ...
    use openduck::acp::server::AcpBuiltinSelection;
    use openduck::acp::server_factory::{AcpServer, AcpServerFactoryConfig};
    use openduck::acp::transport::create_router;
    use openduck::config::paths::Paths;
    ...
    if !require_token && !dangerously_unauthenticated {
        anyhow::bail!(
            "{GOOSE_SERVER_SECRET_KEY_ENV} must be set to start `openduck serve`; pass --dangerously-unauthenticated to run without ACP authentication"
        );
    }
    ...
    if tls {
        ...
        {
            let tls_setup = openduck::acp::transport::tls::setup_tls(
    ...
    match (
        ...
        (Some(file), _, _) => {
            let contents = std::fs::read_to_string(file).unwrap_or_else(|err| {
                eprintln!(
                    "Instruction file not found — did you mean to use openduck run --text?\n{}",
                    err
                );
                std::process::exit(1);
            });
        ...
        (_, _, Some(recipe_name)) => {
            ...
            let recipe_version = crate::recipes::search_recipe::load_recipe_file(recipe_name)
                .ok()
                .and_then(|rf| {
                    openduck::recipe::template_recipe::parse_recipe_content(
                        &rf.content,
...
#[cfg(feature = "local-inference")]
fn print_download_progress(manager: &openduck::download_manager::DownloadManager) {
    let Some(progress) = manager.list_progress().into_iter().find(|progress| {
        progress.status == openduck::download_manager::DownloadStatus::Downloading
    }) else {
        return;
    };
...
async fn handle_local_models_command(command: LocalModelsCommand) -> Result<()> {
    use openduck::providers::local_inference::hf_models;
    use openduck::providers::local_inference::local_model_registry::get_registry;

    openduck::providers::local_inference::configure_huggingface_auth();
    ...
    match command {
            ...
            for model in &results {
                ...
                for variant in &model.variants {
                    ...
                    if variant.supported {
                        println!(
                            "    Download: openduck local-models download '{}'",
                            variant.download_id
                        );
                    }
                }
        ...
        LocalModelsCommand::Download { spec } => {
            ...
            let manager = openduck::download_manager::get_download_manager();
...
pub async fn cli() -> anyhow::Result<()> {
    register_builtin_extensions(openduck_mcp::BUILTIN_EXTENSIONS.clone());
    ...
    match cli.command {
        ...
        Some(Command::Acp {
            builtins,
            enable_scheduler,
        }) => openduck::acp::server::run(builtins, enable_scheduler).await,
        Some(Command::Serve {
            host,
            port,
            tls,
        ...
        Some(Command::ValidateExtensions { file }) => {
            use openduck::agents::validate_extensions::validate_bundled_extensions;
...
mod tests {
    ...
    fn nushell_completion_generation_emits_module() {
        ...
        CompletionShell::Nu.generate(&mut cmd, "openduck", &mut buffer);

        let script = String::from_utf8(buffer).expect("utf8");
        assert!(script.contains("module completions"));
        assert!(script.contains("export extern openduck"));
    ...
    fn term_init_help_mentions_nushell() {
        ...
        assert!(help.contains("openduck term init nu"));
