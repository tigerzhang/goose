...
//!
//! Output: target/man/openduck.1, target/man/openduck-session.1, etc.

use clap::CommandFactory;
use clap_mangen::Man;
use openduck_cli::Cli;
use std::env;
use std::fs;
use std::io::Result;
...
fn generate_see_also(
    ...
    // Always reference the main openduck command if we're not it
    if current_name != "openduck" {
        references.push("openduck".to_string());
    }

    // Reference parent command if exists and not already added
    if let Some(parent) = parent_name {
        if parent != "openduck" && !references.contains(&parent.to_string()) {
    ...
    if current_name == "openduck" {
        ...
        for subcmd in cmd.get_subcommands() {
            ...
            if subcmd_name != "help" && !subcmd.is_hide_set() {
                let full_name = format!("openduck-{}", subcmd_name);
