use anyhow::Result;
use openduck::execution::manager::AgentManager;
use openduck::gateway::manager::GatewayManager;
use std::sync::Arc;
...
pub async fn handle_gateway_start(
    ...
    let mut config = openduck::gateway::GatewayConfig {
        gateway_type,
        platform_config,
        max_sessions: 0,
    };

    let gw = openduck::gateway::create_gateway(&mut config)?;
