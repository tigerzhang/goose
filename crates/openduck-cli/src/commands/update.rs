...
fn sha256_hex(data: &[u8]) -> String {
    ...
    hasher.update(data);
    openduck::utils::bytes_to_hex(hasher.finalize())
}
...
async fn fetch_bundle(client: &reqwest::Client, url: &str) -> Result<serde_json::Value> {
    let resp = client
        .get(url)
        .header("User-Agent", "openduck-cli")
        .send()
        .await
...
pub async fn update(canary: bool, reconfigure: bool) -> Result<()> {
    ...
    #[cfg(not(feature = "disable-update"))]
    {
        ...
        println!("openduck updated successfully (verified with Sigstore SLSA provenance).");

        // --- Reconfigure if requested -------------------------------------------
        if reconfigure {
            println!("Running openduck configure...");
            let status = Command::new(current_exe)
                .arg("configure")
                .status()
                .context("Failed to run openduck configure")?;
            if !status.success() {
                eprintln!("Warning: openduck configure exited with {status}");
            }
        }
...
fn replace_binary(new_binary: &Path, current_exe: &Path) -> Result<()> {
    #[cfg(target_os = "windows")]
    {
        ...
        fs::rename(current_exe, &old_exe).with_context(|| {
            format!(
                "Failed to rename running binary to {}. Try closing OpenDuck Desktop if it's open.",
                old_exe.display()
            )
        })?;
