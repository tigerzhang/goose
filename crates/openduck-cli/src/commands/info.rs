...
use openduck::session::session_manager::{DB_NAME, SESSIONS_FOLDER};
...
use openduck::config::base::CONFIG_YAML_NAME;
use std::fs;
use std::path::Path;
...
async fn check_provider(
    ...
    let model_config = openduck::model_config::model_config_from_user_config(&provider, &model)
        .map_err(|e| ProviderCheckError::InvalidModel(e.to_string()))?;

    let provider_client = openduck::providers::create(&provider, Vec::new())
        .await
        .map_err(|e| {
    ...
    openduck::session_context::with_session_id(
    ...
    if check {
        ...
        match &result {
            ...
            Err(ProviderCheckError::NotConfigured { label, error }) => {
                ...
                print_aligned(
                    "Hint:",
                    &format!("Run '{}'", style("openduck configure").cyan()),
                    label_padding,
                );
            }
