...
use console::style;
use openduck::agents::extension::{ToolInfo, PLATFORM_EXTENSIONS};
use openduck::agents::extension_manager::get_parameter_names;
use openduck::agents::Agent;
use openduck::agents::{extension::Envs, ExtensionConfig};
use openduck::config::declarative_providers::{
    create_custom_provider, remove_custom_provider, CreateCustomProviderParams,
};
use openduck::config::extensions::{
    get_all_extension_names, get_all_extensions, get_enabled_extensions, get_extension_by_name,
    name_to_key, remove_extension, set_extension, set_extension_enabled,
};
use openduck::config::paths::Paths;
use openduck::config::permission::PermissionLevel;
use openduck::config::signup_tetrate::TetrateAuth;
use openduck::config::{
    configure_tetrate, Config, ConfigError, ExperimentManager, ExtensionEntry, GooseMode,
    PermissionManager,
};
#[cfg(feature = "telemetry")]
use openduck::posthog::{get_telemetry_choice, TELEMETRY_ENABLED_KEY};
use openduck::providers::base::ConfigKey;
use openduck::providers::provider_test::test_provider_configuration;
use openduck::providers::{create, providers, retry_operation, RetryConfig};
use openduck::session::SessionType;
use openduck_providers::thinking::ThinkingEffort;
...
pub async fn handle_configure() -> anyhow::Result<()> {
    if !std::io::stdin().is_terminal() {
        anyhow::bail!(
            "openduck configure requires an interactive terminal.\n\
             If you installed via 'curl ... | bash', run 'openduck configure' separately after installation."
        );
    }
...
pub fn configure_telemetry_consent_dialog() -> anyhow::Result<bool> {
    ...
    println!(
        "{}",
        style("or any personal data. You can change this anytime with 'openduck configure'.").dim()
    );
    println!();
...
fn interactive_model_search(
    models: &[String],
    provider_meta: &openduck::providers::base::ProviderMetadata,
...
pub async fn configure_provider_dialog() -> anyhow::Result<bool> {
    ...
    let models_res = retry_operation(&RetryConfig::default(), || async {
        temp_provider
            .fetch_recommended_models(openduck::model_config::global_toolshim())
            .await
    })
    .await;
    ...
    let model: String = match models_res {
        ...
        Ok(_) => {
            let default_model =
                openduck::config::get_var("MODEL").unwrap_or(provider_meta.default_model.clone());
...

    {
        let supports_thinking = match temp_provider.fetch_model_info(&model).await {
            ...
            Err(_) => openduck_providers::model::ModelConfig::new(&model).is_reasoning_model(),
        };
    ...
    let toolshim_enabled = openduck::config::get_var("TOOLSHIM")
        .map(|val| val == "1" || val.to_lowercase() == "true")
        .unwrap_or(false);
    let toolshim_model = openduck::config::get_var("TOOLSHIM_OLLAMA_MODEL");

    match test_provider_configuration(&provider_name, &model, toolshim_enabled, toolshim_model)
        .await
    {
        Ok(()) => {
            openduck::config::set_active_provider(config, &provider_name, &model)?;
...
pub fn toggle_extensions_dialog() -> anyhow::Result<()> {
    for warning in openduck::config::get_warnings() {
...
fn prompt_extension_timeout() -> anyhow::Result<u64> {
    Ok(
        cliclack::input("Please set the timeout for this tool (in secs):")
            .placeholder(&openduck::config::DEFAULT_EXTENSION_TIMEOUT.to_string())
...
fn configure_stdio_extension() -> anyhow::Result<()> {
    ...
    let mut parts = openduck::utils::split_command_args(&command_str)?;
...
pub fn configure_goose_mode_dialog() -> anyhow::Result<()> {
    ...
    if openduck::config::get_var_os("MODE").is_some() {
...
pub fn configure_telemetry_dialog() -> anyhow::Result<()> {
    ...
    if openduck::config::get_var_os("TELEMETRY_OFF").is_some() {
        let _ = cliclack::log::info(
...
pub fn configure_tool_output_dialog() -> anyhow::Result<()> {
    ...
    if openduck::config::get_var_os("CLI_MIN_PRIORITY").is_some() {
        let _ = cliclack::log::info(
...
pub fn configure_keyring_dialog() -> anyhow::Result<()> {
    ...
    if openduck::config::get_var_os("DISABLE_KEYRING").is_some() {
        let _ = cliclack::log::info(
...
pub async fn configure_tool_permissions_dialog() -> anyhow::Result<()> {
    ...
    let model_config =
        openduck::model_config::model_config_from_user_config(&provider_name, &model)?;
...
pub async fn handle_openrouter_auth() -> anyhow::Result<()> {
    use openduck::config::{configure_openrouter, signup_openrouter::OpenRouterAuth};
    ...
    let model_config = match openduck::model_config::model_config_from_user_config(
        "openrouter",
        &configured_model,
    ) {
        Ok(config) => config,
        ...
        Ok(provider) => {
            ...
            match test_result {
                Ok(_) => {
                    ...
                    if !has_developer {
                        set_extension(ExtensionEntry {
                            ...
                            config: ExtensionConfig::Platform {
                                ...
                                display_name: Some(
                                    openduck::config::DEFAULT_DISPLAY_NAME.to_string(),
                                ),
                                bundled: Some(true),
                                available_tools: Vec::new(),
                            },
                    ...
                    cliclack::outro("OpenRouter setup complete! You can now use OpenDuck.")?;
                }
                Err(e) => {
...
pub async fn handle_tetrate_auth() -> anyhow::Result<()> {
    ...
    let configured_model: String = config.get_goose_model()?;
    if let Err(e) =
        openduck::model_config::model_config_from_user_config("tetrate", &configured_model)
    {
        eprintln!("⚠️  Invalid model configuration: {}", e);
        eprintln!("Your settings have been saved. Please check your model configuration.");
        return Ok(());
...
async fn remove_provider() -> anyhow::Result<()> {
    let custom_providers_dir = openduck::config::declarative_providers::custom_providers_dir();
    let custom_providers = if custom_providers_dir.exists() {
        openduck::config::declarative_providers::load_custom_providers(&custom_providers_dir)?
    } else {
        Vec::new()
    };
    ...
    if let Err(e) = openduck::providers::cleanup_provider(selected_id).await {
