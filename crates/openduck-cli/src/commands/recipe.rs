    ...
    match generate_deeplink(recipe_name, params_map) {
        Ok((deeplink_url, recipe)) => match opener(&deeplink_url) {
            Ok(_) => {
                writeln!(
                    out,
                    "{} Opened recipe '{}' in OpenDuck Desktop",
                    style("✓").green().bold(),
                    recipe.title
                )?;
                Ok(())
            ...
            Err(err) => {
                writeln!(
                    out,
                    "{} Failed to open recipe in OpenDuck Desktop: {}",
                    style("✗").red().bold(),
                    err
                )?;
                writeln!(out, "Generated deeplink: {}", deeplink_url)?;
                writeln!(out, "You can manually copy and open the URL above, or ensure OpenDuck Desktop is installed.")?;
...
) -> Result<(String, openduck::recipe::Recipe)> {
    let recipe_file = load_recipe_file(recipe_name)?;
...
"#;
    ...
    fn test_handle_open_opener_fails() {
        ...
        assert!(result.is_err());
        assert!(output.contains("Failed to open recipe in OpenDuck Desktop"));
