...
use chrono;
use openduck::config::Config;
use openduck::conversation::message::{Message, MessageContent, MessageMetadata};
...
goose_preexec() {
    [[ "$1" =~ ^(goose|openduck|duck)\ term ]] && return
...
function goose_preexec --on-event fish_preexec
    string match -q -r '^(goose|openduck|duck) term' -- $argv[1]; and return
...
if (($env | get -o GOOSE_NU_PREEXEC_INSTALLED | default false) != true) {
    ...
    $env.config.hooks.pre_execution = (
        ...
        | append {||
            ...
            if ($line =~ '^(goose|openduck|duck) term(\s|$)') {
                return
            }
            if ($line =~ '^(@goose|@g)(\s|$)') {
                return
...
Set-PSReadLineKeyHandler -Chord Enter -ScriptBlock {{
    ...
    if ($line -notmatch '^(goose|openduck|duck) term' -and $line -notmatch '^(@goose|@g)($|\s)') {{
...
pub async fn handle_term_init(
    ...
    let session = match named_session {
        ...
        None => {
            let session = session_manager
                .create_session(
                    working_dir,
                    "OpenDuck Term Session".to_string(),
    ...
    let goose_bin = std::env::current_exe()
        ...
        .unwrap_or_else(|_| "openduck".to_string());
...
pub async fn handle_term_log(command: String) -> Result<()> {
    let session_id = std::env::var("AGENT_SESSION_ID").map_err(|_| {
        anyhow!(
            "AGENT_SESSION_ID not set. Initialize terminal integration with `openduck term init <shell>` and reload your shell first."
        )
    })?;
    ...
    let session_id = std::env::var("AGENT_SESSION_ID").map_err(|_| {
        anyhow!(
            "AGENT_SESSION_ID not set.\n\n\
             Initialize terminal integration with `openduck term init <shell>` in your shell profile, \
...
pub async fn handle_term_info() -> Result<()> {
    ...
    let config = openduck::config::Config::global();
