...
async fn ensure_working_provider(
    ...
    Ok(Some(Message::assistant().with_text(format!(
        "**Goose Doctor**\n\n{}\n\n\
         No working provider found. Run `openduck configure` to set one up.",
        preamble,
    ))))
}
...
async fn try_create_and_test(
    provider_name: &str,
    model_name: &str,
) -> Result<(Arc<dyn Provider>, openduck_providers::model::ModelConfig), ProviderError> {
    let model_config =
...
async fn try_other_models(
    ...
    skip_model: &str,
    log: &mut Vec<String>,
) -> Option<(Arc<dyn Provider>, openduck_providers::model::ModelConfig)> {
...
fn describe_error(e: &ProviderError) -> String {
    match e {
        ProviderError::NotConfigured => {
            "Provider is not configured. Run `openduck configure` to set it up.".to_string()
        }
        ProviderError::Authentication(_) => {
            "Authentication failed — check your API key. Run `openduck configure` to update it."
                .to_string()
        }
