...
use openduck_providers::thinking::{ThinkingEffort, ThinkingEffortSupport};
use regex::Regex;
use rmcp::model::{
...
fn normalize_legacy_provider_thinking_effort(
    mut model_config: openduck_providers::model::ModelConfig,
    effort_support: &ThinkingEffortSupport,
) -> openduck_providers::model::ModelConfig {
...
pub struct ReplyContext {
    ...
    pub model_config: openduck_providers::model::ModelConfig,
}
...
impl fmt::Display for GoosePlatform {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            GoosePlatform::GooseCli => write!(f, "openduck-cli"),
            GoosePlatform::GooseDesktop => write!(f, "openduck-desktop"),
        }
    }
}
        ...
        let tool_call_cut_off = match Config::global().get_param::<usize>("GOOSE_TOOL_CALL_CUTOFF")
        {
            ...
            Err(_) => {
                let context_limit = match self.provider().await {
                    ...
                    Err(_) => openduck_providers::model::DEFAULT_CONTEXT_LIMIT,
                };
...
mod tests {
    ...
    async fn tool_dispatch_records_gen_ai_span_attributes() {
        use openduck_test_support::otel::clear_otel_env;
        use rmcp::object;
    ...
    impl crate::providers::base::Provider for ActionRequiredProvider {
        ...
        async fn stream(
            &self,
            _: &openduck_providers::model::ModelConfig,
            _: &str,
        ...
        fn thinking_effort_support(&self) -> ThinkingEffortSupport {
            if self.applies_effort {
                ThinkingEffortSupport::Options(
                    openduck_providers::thinking::ThinkingEffortCapability {
                        option_id: "effort".to_string(),
                        values: vec![openduck_providers::thinking::ThinkingEffortOption {
    ...
    async fn effort_test_agent(
        ...
        agent
            .update_provider(
                provider.clone(),
                openduck_providers::model::ModelConfig::new("mock-model"),
                &session.id,
            )
            .await
            .unwrap();
    ...
    async fn update_provider_replaces_harness_only_effort_for_legacy_provider() {
        ...
        let model_config =
            openduck_providers::model::ModelConfig::new("mock-model").with_merged_request_params(
