...
impl Provider for ResolvedModelProvider {
    ...
    ) -> Result<crate::providers::base::MessageStream, openduck_providers::errors::ProviderError>
    {
        self.inner
    ...
    async fn get_context_limit(
        &self,
        model_config: &ModelConfig,
    ) -> Result<usize, openduck_providers::errors::ProviderError> {
    ...
    async fn fetch_model_info(
        &self,
        model_name: &str,
    ) -> Result<openduck_providers::base::ModelInfo, openduck_providers::errors::ProviderError>
    {
...
async fn test_pipeline_with_components(
    ...
    let session = session_manager
        ...
        .await?;
    let model_config = ModelConfig::new(openduck_providers::openai::OPEN_AI_DEFAULT_MODEL)
...
async fn build_test_pipeline(
    ...
    let api_client = openduck_providers::api_client::ApiClient::new_with_tls(
        api.uri(),
        openduck_providers::api_client::AuthMethod::NoAuth,
        None,
    )?;
    let provider: Arc<dyn Provider> = Arc::new(
        openduck_providers::openai::OpenAiProviderBuilder::new(api_client)
            .name(provider_name)
