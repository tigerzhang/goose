...
use openduck_agent::machine::{EffectHandler, EffectUsage, MachineSession, SessionLoader};
use openduck_agent::operation::{ConversationEffect, Emitter, MachineEffect};
...
impl EffectUsage<GooseEffect> for SessionManager {
    fn usage(
        &self,
        effect: &GooseEffect,
    ) -> Option<openduck_providers::conversation::token_usage::Usage> {
        match effect {
            GooseEffect::RecordUsage(usage)
            | GooseEffect::ReplaceConversation {
                usage: Some(usage), ..
...
pub(crate) async fn run(
    ...
    let mut turn_usage = openduck_providers::conversation::token_usage::Usage::default();
    loop {
        let session = runtime.load(session_id).await?;
        let Some(mut result) = machine.step(&session, emit).await? else {
            break;
        };
        tracing::debug!(target: "openduck::state_machine", step = result.applied_step, "applied step");
