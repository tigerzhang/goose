...
use openduck_providers::api_client::{ApiClient, AuthMethod, TlsConfig};
...
use openduck_providers::ollama::fetch_ollama_model_names;
use openduck_providers::openai::OpenAiProvider;
use rmcp::model::Tool;
...
fn build_ollama_api_client(
    ...
    let normalized_base_url = openduck_providers::openai::ensure_url_scheme(&config.base_url);
        ...
        Box::pin(async {
            anyhow::bail!(
                "Ollama Cloud must be configured as a declarative provider. \
                 Run `openduck configure` to set it up."
            )
        })
    }
}
