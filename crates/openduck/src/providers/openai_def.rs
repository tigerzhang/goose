...
use openduck_providers::base::ProviderDescriptor;
...
use openduck_providers::api_client::{ApiClient, AuthMethod};
use openduck_providers::openai::{
...
impl ProviderDescriptor for OpenAiProviderDef {
    fn metadata() -> openduck_providers::base::ProviderMetadata {
...
pub async fn from_env(
    tls_config: Option<openduck_providers::api_client::TlsConfig>,
    ...
    //   3. OPENAI_HOST from config file — persisted by `openduck configure`
            ...
            match e {
                ConfigError::NotFound(_) => {
                    if config.requires_auth {
                        anyhow::bail!(
                            "Required API key {} is not set. Configure it via `openduck configure` or set the {} environment variable.",
                            config.api_key_env,
                            config.api_key_env
                        );
                    }
...
pub fn from_custom_config(
    ...
    openduck_providers::openai::from_declarative_config(
        config,
        tls_config,
