...
fn agent_capabilities_meta() -> Option<Meta> {
    ...
    crate::acp::brand_meta::insert_brand_meta(&mut meta, serde_json::Value::Object(goose));
    Some(meta)
}
...
struct ClientCapabilitiesMeta {
    ...
    #[serde(default)]
    openduck: Option<GooseClientCapabilities>,
}
...
fn extract_client_capabilities_meta(args: &InitializeRequest) -> Option<GooseClientCapabilities> {
    let parsed: ClientCapabilitiesMeta =
        args.client_capabilities.meta.as_ref().and_then(|meta| {
            serde_json::from_value(serde_json::Value::Object(meta.clone())).ok()
        })?;
    parsed.openduck.or(parsed.goose)
}
...
fn extract_use_login_shell_path(args: &InitializeRequest) -> bool {
    args.meta
        .as_ref()
        .and_then(|meta| {
            meta.get("openduck/useLoginShellPath")
                .or_else(|| meta.get("goose/useLoginShellPath"))
        })
...
fn resolve_default_provider_model_config(
    config: &Config,
) -> Result<(String, openduck_providers::model::ModelConfig), agent_client_protocol::Error> {
...
async fn resolve_provider_default_model_config(
    provider_name: &str,
) -> Result<openduck_providers::model::ModelConfig, agent_client_protocol::Error> {
...
impl GooseAcpAgent {
    async fn on_initialize(
        ...
        let goose_client_capabilities = extract_client_capabilities_meta(&args);
        ...
        Ok(InitializeResponse::new(args.protocol_version)
            ...
            .auth_methods(vec![AuthMethod::Agent(
                AuthMethodAgent::new("goose-provider", "Configure Provider")
                    .description("Run `openduck configure` to set up your AI provider and API key"),
            )]))
    }
...
mod tests {
    ...
    use openduck_providers::conversation::token_usage::Usage as TokenUsage;
    use openduck_providers::thinking::{
...
```",
    ...
    fn test_build_usage_update_clamps_negative_used_to_zero() {
        ...
        session.model_config = Some(
            openduck_providers::model::ModelConfig::new("test-model")
    ...
    fn test_goose_custom_notifications_capability_defaults_to_false() {
        ...
        let goose_client_capabilities = extract_client_capabilities_meta(&request);
    ...
    fn test_agent_capabilities_advertise_recipe_parameter_scopes() {
        let meta = agent_capabilities_meta().expect("agent capabilities meta");
        ...
        assert_eq!(
            meta.get("openduck")
                .and_then(|openduck| openduck.get("recipeParameterScopes").cloned()),
            expected
        );
    }
    ...
    #[test]
    fn test_openduck_client_meta_is_preferred_over_legacy_goose() {
        let mut goose_meta = serde_json::Map::new();
        goose_meta.insert(
            "customNotifications".to_string(),
            serde_json::Value::Bool(false),
        );
        let mut openduck_meta = serde_json::Map::new();
        openduck_meta.insert(
            "customNotifications".to_string(),
            serde_json::Value::Bool(true),
        );
        let mut meta = serde_json::Map::new();
        ...
        meta.insert(
            "openduck".to_string(),
            serde_json::Value::Object(openduck_meta),
        );
        ...
        assert!(extract_client_supports_goose_custom_notifications(
            extract_client_capabilities_meta(&request).as_ref()
        ));
    }
    ...
    async fn asynchronous_provider_effort_update_is_forwarded_to_client() {
        ...
        session_agent
            .update_provider(
                provider.clone(),
                openduck_providers::model::ModelConfig::new("gpt-4o").with_merged_request_params(
