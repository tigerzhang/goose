...
async fn forward_child_stderr(mut stderr: tokio::process::ChildStderr) {
    ...
    loop {
        match stderr.read(&mut chunk).await {
            ...
            Err(e) => {
                tracing::debug!(target: "openduck::acp::child::stderr", error = %e, "stderr read error");
                break;
            }
        }
    }
...
fn emit_stderr_line(line: &mut Vec<u8>) {
    ...
    tracing::info!(target: "openduck::acp::child::stderr", "{}", String::from_utf8_lossy(trimmed));
    line.clear();
}
...
mod tests {
    ...
    async fn get_context_limit_surfaces_captured_context_size() {
        ...
        assert_eq!(
            ...
            openduck_providers::model::DEFAULT_CONTEXT_LIMIT
        );
