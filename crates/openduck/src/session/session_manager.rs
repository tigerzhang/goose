...
fn deserialize_session_model_config(
    ...
    if provider_name == Some(openduck_providers::azure_foundry::AZURE_FOUNDRY_PROVIDER_NAME) {
        #[derive(Deserialize)]
...
mod tests {
    ...
    use openduck_providers::conversation::token_usage::{CostSource, ProviderUsage};
    ...
    fn azure_session_model_config_preserves_suffixed_deployment_id() {
        ...
        let config = deserialize_session_model_config(
            Some(openduck_providers::azure_foundry::AZURE_FOUNDRY_PROVIDER_NAME),
            &json,
        )
        .unwrap();
    ...
    fn azure_session_model_config_preserves_explicit_thinking_effort() {
        ...
        assert_eq!(
            ...
            Some(openduck_providers::thinking::ThinkingEffort::Low)
        );
    }
    ...
    fn non_azure_session_model_config_keeps_suffix_normalization() {
        let config = deserialize_session_model_config(
            Some(openduck_providers::openai::OPEN_AI_PROVIDER_NAME),
    ...
    impl Provider for NamingTestProvider {
        ...
        ) -> std::result::Result<MessageStream, openduck_providers::errors::ProviderError> {
