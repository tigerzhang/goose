...
use fs_err as fs;
use openduck::acp::server::AcpProviderFactory;
use openduck::config::base::CONFIG_YAML_NAME;
use openduck::config::GooseMode;
use openduck::session::{EnabledExtensionsState, SessionManager};
use openduck_test_support::{McpFixture, FAKE_CODE, TEST_IMAGE_B64, TEST_MODEL};
...
pub async fn run_new_session_uses_current_config_mode<C: Connection>() {
    ...
    let config_path = temp_dir
        .path()
        .join(openduck::config::base::CONFIG_YAML_NAME);
    fs::write(
        &config_path,
    ...
    let global_config_path =
        openduck::config::paths::Paths::config_dir().join(openduck::config::base::CONFIG_YAML_NAME);
    fs::write(
        &global_config_path,
        format!("GOOSE_MODEL: {TEST_MODEL}\nGOOSE_PROVIDER: openai\nGOOSE_MODE: auto\n"),
    )
