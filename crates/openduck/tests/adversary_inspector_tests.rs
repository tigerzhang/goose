...
use openduck::conversation::message::{Message, MessageContent, ToolRequest};
use openduck::security::adversary_inspector::AdversaryInspector;
use openduck::tool_inspection::ToolInspector;
...
async fn test_adversary_disabled_without_config_file() {
    ...
    let inspector = AdversaryInspector::with_config_dir(
        provider,
        Arc::new(openduck::session::SessionManager::new(
...
async fn test_adversary_enabled_default_tools() {
    ...
    assert!(matches!(
        results[0].action,
        openduck::tool_inspection::InspectionAction::Allow
    ));
