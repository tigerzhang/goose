...
use openduck::agents::extension_manager::{ExtensionManager, ExtensionManagerCapabilities};
...
impl openduck::providers::base::ProviderDescriptor for MockProvider {
...
async fn test_replayed_session(
    ...
    let _env = env_lock::lock_env([
        ...
        ("GOOSE_WORKING_DIR", Some("/tmp/openduck_test")),
    ]);

    // Setup test file for developer extension tests
    let test_file_path = "/tmp/openduck_test/goose.txt";
    ...
    let session_manager = Arc::new(openduck::session::SessionManager::new(
