...
use openduck::conversation::message::{Message, MessageMetadata};
use openduck::custom_requests::{GetSessionInfoRequest, GetSessionInfoResponse};
use openduck::recipe::{Recipe, Settings};
...
fn test_new_session_response_precedes_available_commands() {
    run_test(async {
        ...
        let (transport, _handle, _permission_manager) = spawn_acp_server_in_process(
            ...
            GooseMode::default(),
            None,
            openduck_test_support::TEST_MODEL,
            true,
        )
        .await;
