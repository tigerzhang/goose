...
use openduck::acp::ACP_CURRENT_MODEL;
use openduck::agents::{
...
use openduck::permission::{Permission, PermissionConfirmation};
use openduck::providers::anthropic::ANTHROPIC_DEFAULT_MODEL;
use openduck::providers::azure::AZURE_DEFAULT_MODEL;
use openduck::providers::base::Provider;
#[cfg(feature = "aws-providers")]
use openduck::providers::bedrock::BEDROCK_DEFAULT_MODEL;
use openduck::providers::claude_code::CLAUDE_CODE_DEFAULT_MODEL;
use openduck::providers::codex::CODEX_DEFAULT_MODEL;
use openduck::providers::create_with_named_model;
use openduck::providers::google::GOOGLE_DEFAULT_MODEL;
use openduck::providers::litellm::LITELLM_DEFAULT_MODEL;
use openduck::providers::openai::OPEN_AI_DEFAULT_MODEL;
#[cfg(feature = "aws-providers")]
use openduck::providers::sagemaker_tgi::SAGEMAKER_TGI_DEFAULT_MODEL;
use openduck::providers::snowflake::SNOWFLAKE_DEFAULT_MODEL;
use openduck::providers::xai::XAI_DEFAULT_MODEL;
use openduck::session::{SessionManager, SessionType};
use openduck_providers::databricks::DATABRICKS_DEFAULT_MODEL;
use openduck_providers::errors::ProviderError;
use openduck_providers::thinking::ThinkingEffortSupport;
use openduck_test_support::{
...
impl ProviderFixture {
    async fn setup(config: &ProviderTestConfig, mode: GooseMode) -> Result<Self> {
        ...
        let model_config = openduck::model_config::model_config_from_user_config(
            &config.name.to_lowercase(),
            config.model_name,
        )?;
    ...
    async fn tool_roundtrip(
        &self,
        prompt: &str,
        model_config: Option<openduck_providers::model::ModelConfig>,
    ) -> Result<Message> {
        let tools = self
            .agent
            .extension_manager
        ...
        let (response1, _) = openduck::session_context::with_session_id(
        ...
        let ctx = openduck::agents::ToolCallContext::new(
        ...
        let (response2, _) = openduck::session_context::with_session_id(
            Some(self.session_id.clone()),
            self.provider.complete(
                &model_config,
                &system,
    ...
    async fn test_basic_response(&self) -> Result<()> {
        ...
        let (response, _) = openduck::session_context::with_session_id(
            Some(self.session_id.clone()),
            self.provider.complete(
                &model_config,
    ...
    async fn test_context_length_exceeded_error(&self) -> Result<()> {
        ...
        let result = openduck::session_context::with_session_id(
            Some(self.session_id.clone()),
            self.provider.complete(
                &model_config,
                "You are a helpful assistant.",
    ...
    async fn test_image_content_support(&self) -> Result<()> {
        let image_config = self.image_model.as_ref().map(|model| {
            openduck_providers::model::ModelConfig::new(model).with_canonical_limits(&self.name)
        });
        let response = self
            .tool_roundtrip(
    ...
    async fn test_model_switch(&self) -> Result<()> {
        ...
        let alt_config =
            openduck_providers::model::ModelConfig::new(alt).with_canonical_limits(&self.name);
