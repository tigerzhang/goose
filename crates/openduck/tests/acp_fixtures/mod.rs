...
use fs_err as fs;
use openduck::acp::server::{serve, AcpProviderFactory, GooseAcpAgent, GooseAcpAgentOptions};
pub use openduck::acp::{map_permission_response, PermissionDecision};
...
use openduck::config::{GooseMode, PermissionManager};
use openduck::providers::api_client::{ApiClient, AuthMethod as ApiAuthMethod};
use openduck::providers::base::Provider;
use openduck::providers::openai::OpenAiProvider;
use openduck::scheduler::{ScheduledJob, SchedulerError, ValidatedScheduleRecipe};
use openduck::scheduler_trait::SchedulerTrait;
use openduck::session::Session as GooseSession;
use openduck::session_context::SESSION_ID_HEADER;
use openduck_test_support::{ExpectedSessionId, TEST_MODEL};
...
fn write_global_test_config(config_path: &Path, openai_base_url: &str) {
    ...
    let global_config_path = global_config_dir.join(openduck::config::base::CONFIG_YAML_NAME);
...
pub async fn spawn_acp_server_in_process(
    ...
    let config_path = data_root.join(openduck::config::base::CONFIG_YAML_NAME);
    ...
    let agent = GooseAcpAgent::new(GooseAcpAgentOptions {
        provider_factory,
        builtin_selection: openduck::acp::server::AcpBuiltinSelection {
