...
use openduck::config::{Config, ConfigError};
use openduck::providers::base::{MessageStream, Provider};
use openduck::providers::inventory::ProviderInventoryService;
use openduck::session::session_manager::SessionStorage;
use openduck_providers::errors::ProviderError;
use openduck_providers::model::ModelConfig;
use openduck_test_support::EnforceSessionId;
...
impl Provider for MockProvider {
    ...
    async fn stream(
        ...
        _system: &str,
        _messages: &[openduck::conversation::message::Message],
...
fn mock_provider_factory() -> openduck::acp::server::AcpProviderFactory {
    Arc::new(
...
fn write_config(config_dir: &std::path::Path) {
    ...
    std::fs::write(
        config_dir.join(openduck::config::base::CONFIG_YAML_NAME),
