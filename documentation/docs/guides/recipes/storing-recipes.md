...
This guide covers storing, organizing, and finding OpenDuck recipes when you need to access them again later.



:::info Desktop UI vs CLI

- **OpenDuck Desktop** has a visual Recipe Library for browsing and managing saved recipes

- **OpenDuck CLI** stores recipes as files that you find using file paths or environment variables
...
| **Global** | OpenDuck config `recipes/` (Linux: `~/.config/OpenDuck/recipes/`; legacy `~/.config/goose/recipes/`) | All projects and sessions | Personal workflows, general-purpose recipes |

| **Local** | `YOUR_WORKING_DIRECTORY/.openduck/recipes/` (legacy `.goose/recipes/`) | Only when working in that project | Project-specific workflows, team recipes |
...
<Tabs groupId="interface">
  ...
  <TabItem value="desktop" label="OpenDuck Desktop" default>
...
:::
  ...
  <TabItem value="cli" label="OpenDuck CLI">
    ...
    * Local project recipes: `/recipe .openduck/recipes/my-recipe.yaml`
...
<Tabs groupId="interface">
  ...
  OpenDuck Desktop accepts `.yaml`, `.yml`, and `.json` files, but **the CLI only supports `.yaml` and `.json`**. For full compatibility across both interfaces, avoid `.yml` extensions.
  ...
  <TabItem value="cli" label="OpenDuck CLI">
    ...
    Recipe import is only available in OpenDuck Desktop.
...
Use the `openduck recipe list` command to find all available recipes from multiple sources:
...
hello-world - A sample recipe demonstrating basic usage - local: ~/.config/OpenDuck/recipes/hello-world.yaml
...
Available recipes:
  ...
  hello-world - A sample recipe demonstrating basic usage - local: ~/.config/OpenDuck/recipes/hello-world.yaml
    ...
    Path: /Users/username/.config/OpenDuck/recipes/hello-world.yaml
...
The `openduck recipe list` command is the recommended way to find recipes as it automatically searches all configured sources and provides consistent formatting.
...
Once you've located your recipe file, [run the recipe](/docs/guides/recipes/session-recipes#run-a-recipe) or [open it in OpenDuck Desktop](/docs/guides/goose-cli-commands#recipe).



:::tip Format Compatibility

The CLI can run recipes saved from OpenDuck Desktop without any conversion. Both CLI-created and Desktop-saved recipes work with all recipe commands.
